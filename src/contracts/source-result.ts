...
export const sourceStampSchema = z.object({
...
export const profileSummarySchema = z.object({
  ...
  resolutionSource: z.enum(["explicit", "environment", "sticky", "platform-default", "unavailable"]),
...
export const workspaceFileSchema = workspaceEntrySchema.extend({
...
export const hermesJobSchema = z.object({
  id: z.string().min(1).max(200),
...
export const overviewSnapshotSchema = z.object({
  observedAt: z.string().min(1).max(80),
  profile: overviewSectionStatusSchema.extend({
    profile: z.string().min(1).max(100),
    homeLabel: z.string().min(1).max(1_024),
    configState: z.enum(["ready", "unavailable", "error"]),
    model: z.string().min(1).max(200).nullable(),
  ...
  conversations: overviewSectionStatusSchema.extend({
    items: z.array(z.object({
      id: z.string().min(1).max(500),
      title: z.string().min(1).max(500),
      preview: z.string().max(1_000),
  ...
  system: overviewSectionStatusSchema.extend({
    items: z.array(z.object({
      id: z.string().min(1).max(100),
      title: z.string().min(1).max(120),
      label: z.string().min(1).max(120),
  ...
  jobs: overviewSectionStatusSchema.extend({
  ...
  workspace: overviewSectionStatusSchema.extend({
