...
function matchesCollectionQuery(item: NonNullable<SystemSource["collection"]>["items"][number], query: string): boolean {
  const normalized = query.trim().toLocaleLowerCase();
  if (!normalized) return true;
  return [item.name, item.description, item.category, item.status, item.path ?? ""]
    .some((value) => value.toLocaleLowerCase().includes(normalized));
}

export function SystemBrowser({ sources }: { sources: SystemSource[] }) {
  ...
  const selectedCollectionItem = selected?.collection?.items.find((item) => item.id === selectedCollectionItemId);
  ...
  const loadSkill = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/system/context?id=${encodeURIComponent(id)}`, {
        cache: "no-store",
        signal: controller.signal,
      });
      const payload: unknown = await response.json();
      const parsed = systemSourceSchema.safeParse(payload);
      if (!response.ok || !parsed.success || parsed.data.collection) throw new Error("invalid skill preview");
      if (!controller.signal.aborted) {
        setSkillPreview(parsed.data);
        setSkillPreviewState("idle");
      }
    } catch {
      if (!controller.signal.aborted) setSkillPreviewState("error");
    }
  };
  ...
  return (
    <div className="inspection-grid">
      <section className="index-pane" aria-label="System sources">
        <div className="pane-label">CONTEXT INDEX / {sources.length.toString().padStart(2, "0")}</div>
          ...
          {sources.map((source, index) => (
            <li key={source.id}>
              ...
              >
                ...
                <span><strong>{source.title}</strong><small>{source.category}</small></span>
              </button>
            </li>
          ))}
        </ul>
      ...
      <article className="preview-pane" aria-labelledby="system-preview-title">
        <div className="preview-toolbar">
          <div>
            ...
            <h2 id="system-preview-title" ref={previewTitleRef} tabIndex={-1}>{selected.title}</h2>
          </div>
        ...
        {selected.stamp.state === "ready" && selected.collection ? (
          <>
            ...
            <ul className="capability-list" aria-label={`${selected.title} list`}>
              {collectionItems.map((item) => (
                <li key={item.id}>
                  <button
                    type="button"
                    className={cn("capability-row", selectedCollectionItemId === item.id && "is-selected")}
                    onClick={() => selectCollectionItem(item.id)}
                    aria-pressed={selectedCollectionItemId === item.id}
                  >
                    <span><strong>{item.name}</strong><small>{item.description}</small></span>
                    <span className="capability-meta"><em>{item.category}</em><b>{item.status}</b></span>
                  </button>
                </li>
              ))}
            </ul>
            {collectionItems.length === 0 ? <p className="state-message">No matching entries.</p> : null}
            ...
            {skillPreviewState === "error" ? (
              <SourceState kind="error" detail="The selected skill manifest could not be safely loaded." />
            ) : null}
            {skillPreview ? (
              <section className="capability-preview" aria-labelledby="skill-preview-title">
                <h3 className="sr-only" id="skill-preview-title">{skillPreview.title} instructions</h3>
                <SearchField
                ...
                <SafeMarkdown
                  ...
                  query={previewQuery}
                />
                {skillPreview.stamp.truncated ? <p className="truncation-note">Preview is bounded.</p> : null}
              </section>
            ) : null}
            {selected.collection.kind === "tools" && selectedCollectionItem ? (
              <section className="capability-preview tool-preview" aria-labelledby="tool-preview-title">
                <p className="eyebrow">{selectedCollectionItem.category} / {selectedCollectionItem.status}</p>
          ...
          <>
            <SearchField label="Search this document" value={query} onChange={setQuery} resultCount={matches} />
            <SafeMarkdown ariaLabel={`${selected.title} preview`} content={selected.content} query={query} />
          </>
        ) : (
          <SourceState
        ...
        {selected.stamp.truncated ? <p className="truncation-note">Preview is bounded. Additional source content is not shown.</p> : null}
      </article>
      ...
      <div className="ledger-column">
        ...
        <section className="boundary-note">
          ...
          <div><strong>Allowlisted preview</strong><p>Credentials and raw provider settings are never sent to the browser.</p></div>
        </section>
