import { Activity, ArrowUpRight, CalendarClock, Database, FileText, MessagesSquare } from "lucide-react";
...
function sectionFailure(section: OverviewSectionStatus) {
  ...
  return <SourceState kind={section.state} {...(section.message ? { detail: section.message } : {})} />;
}
...
export function OverviewDashboard({ snapshot }: { snapshot: OverviewSnapshot }) {
  const { conversations, jobs, profile, system, workspace } = snapshot;
  const conversationCount = conversations.state === "ready"
    ? `${conversations.items.length}${conversations.hasMore ? "+" : ""}`
    : "—";
  const workspaceCount = workspace.loadedCount === null
    ? "Unavailable"
    : workspace.truncated ? `${workspace.loadedCount} shown / partial` : `${workspace.loadedCount} visible entries`;
  const modelAvailable = profile.state === "ready" && profile.configState === "ready";
  const statusItems = [
    {
      label: "Profile",
      value: profile.state === "ready" ? profile.profile : "Unavailable",
      detail: profile.state === "ready" ? profile.homeLabel : profile.message ?? "Hermes home unavailable",
    },
    {
      ...
      detail: modelAvailable
        ...
        : profile.configState === "error" ? "Config read failed" : "Config unavailable",
    },
    {
      label: "Conversations",
      value: conversationCount,
      detail: conversations.state === "ready"
    ...
    },
    {
      ...
      detail: workspace.state === "ready" ? `Approved workspace · ${workspaceCount}` : workspace.message ?? "Source unavailable",
    },
  ];
    ...
    <div className="page-wrap">
      ...
      <div className="overview-grid">
        <section className="overview-primary">
          <SectionHeading index="A" title="Conversations" detail={`Recent activity · ${observedDetail(conversations.observedAt)}`} />
          {sectionFailure(conversations) ?? (conversations.items.length === 0
                  ...
                  {conversations.items.map((conversation) => (
                    <div key={conversation.id} className="register-row overview-register-row">
                      ...
                      <span>
                        ...
                        <small>{conversation.source} · {conversation.preview}</small>
                      </span>
          ...
          <Link href="/conversations" className="button button-outline overview-register-link">
            Conversations <ArrowUpRight aria-hidden="true" size={14} />
          </Link>
        </section>
        <section className="overview-secondary">
          <SectionHeading index="B" title="System" detail="Five core sources · independently observed" />
              ...
              {system.items.map((source) => (
                <div key={source.id}>
                  ...
                  <span>
                    ...
                    <small>{source.label} · {source.freshnessLabel} {formatShanghaiTime(source.freshnessAt)}</small>
                  </span>
          ...
          <Link href="/system" className="button button-outline overview-register-link">
        ...
        <section className="overview-secondary">
          <SectionHeading
            index="C"
            title="Jobs"
            detail={jobs.total === null ? observedDetail(jobs.observedAt) : `${jobs.total} total · ${observedDetail(jobs.observedAt)}`}
          />
          {sectionFailure(jobs) ?? (jobs.total === 0
            ? <SourceState kind="empty" detail="The job source is available but contains no definitions." />
            : (
                <>
                  <dl className="metric-block">
                    <div><dt>Enabled</dt><dd>{jobs.enabled}</dd></div>
                    <div><dt>Paused</dt><dd>{jobs.paused}</dd></div>
                    <div><dt>Failed last run</dt><dd>{jobs.failedLastRun}</dd></div>
                  </dl>
                  ...
                  <div className="next-event">
                    ...
                    <span>
                      ...
                      <em className="mono">{jobs.nextEvent ? formatShanghaiTime(jobs.nextEvent.nextRun) : "No active next-run timestamp"}</em>
                    </span>
                  </div>
                </>
              ))}
          <Link href="/jobs" className="button button-outline overview-register-link">
        ...
        <section className="overview-primary">
          ...
          />
          {sectionFailure(workspace) ?? (workspace.items.length === 0
            ? (
                <SourceState
                  kind="empty"
                  detail={workspace.loadedCount === 0
            ...
            : (
                <>
                  ...
                  {workspace.truncated ? <p className="state-message">The root listing is partial; counts are not presented as exact.</p> : null}
                </>
              ))}
          <Link href="/files" className="button button-outline overview-register-link">
      ...
      <footer className="fixture-banner">
        ...
        <span><strong>Live local snapshot</strong> Read-only sources are composed independently and retain their own observation times.</span>
      </footer>
    </div>
  );
}
