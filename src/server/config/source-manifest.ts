...
const jobDefinitionColumnsSchema = z
  .object({
    id: sqlIdentifier,
    name: sqlIdentifier,
...
const jobScheduleColumnsSchema = z
  .object({
...
const jobExecutionColumnsSchema = z
  .object({
    id: sqlIdentifier,
    jobId: sqlIdentifier,
...
const sourceManifestSchema = z
  ...
  .superRefine((manifest, context) => {
    ...
    if (Boolean(conversation.promptTable) !== Boolean(conversation.promptColumns)) {
      context.addIssue({
        code: "custom",
        message: "Prompt table and columns must be configured together.",
      });
    }
    if (conversation.promptTable && !conversation.sessionColumns.promptHash) {
      context.addIssue({
        code: "custom",
        message: "Prompt hash is required when a prompt table is configured.",
      });
    }
  });
