...
describe("read-only SQLite foundation", () => {
  ...
  beforeEach(() => {
    ...
    writer.exec("CREATE TABLE fixture_records (id INTEGER PRIMARY KEY, label TEXT); INSERT INTO fixture_records (label) VALUES ('safe');");
    writer.close();
  });
  ...
  it("opens an existing database for bounded reads and rejects writes", async () => {
    ...
    const statement = database.prepare<[], { label: string }>("SELECT label FROM fixture_records LIMIT 1");
    ...
    expect(() => database.prepare("INSERT INTO fixture_records (label) VALUES ('blocked')")).toThrowError(
      expect.objectContaining({ code: "source_malformed" }),
    );
    await session.close();

    const verifier = new Database(databasePath, { readonly: true });
    expect(verifier.prepare("SELECT label FROM fixture_records ORDER BY id").pluck().all()).toEqual(["safe"]);
    verifier.close();
  });

  it("requires the database file to already exist", async () => {
    await expect(openReadOnlyDatabase(path.join(fixtureRoot, "missing.sqlite"), {
      protectedSourceRoots: [fixtureRoot],
    })).rejects.toMatchObject({ code: "missing_source" });
  });

  it("requires at least one protected source root", async () => {
    await expect(openReadOnlyDatabase(databasePath, {
  ...
  it("keeps the business database immutable while SQLite creates missing WAL coordination files", async () => {
    ...
    const row = await withReadOnlyDatabase(
      databasePath,
      (database) => database.prepare("SELECT label FROM fixture_records LIMIT 1").get() as { label: string },
      { protectedSourceRoots: [fixtureRoot] },
    );
  ...
  it("keeps the business database and live WAL immutable during a coordinated read", async () => {
    ...
    try {
      const labels = await withReadOnlyDatabase(
        databasePath,
        (database) => (
          database.prepare("SELECT label FROM fixture_records ORDER BY id").all() as Array<{ label: string }>
        ).map((row) => row.label),
        { protectedSourceRoots: [fixtureRoot] },
      );
  ...
  it("keeps multi-step reads on one consistent WAL snapshot", async () => {
    ...
    try {
      const counts = await withReadOnlyDatabase(
        ...
        (database) => {
          const count = () => (
            database.prepare<[], { total: number }>("SELECT COUNT(*) AS total FROM fixture_records").get()?.total
          );
          const before = count();
  ...
  it("maps busy errors to a safe code", () => {
    expect(classifySourceError({ code: "SQLITE_BUSY", message: `locked at ${databasePath}` })).toBe("source_busy");
  });

  it.each(["SQLITE_NOTADB", "SQLITE_FORMAT", "SQLITE_CORRUPT", "SQLITE_CORRUPT_VTAB"])(
    "maps malformed SQLite error %s to a safe code",
    (code) => expect(classifySourceError({ code, message: `invalid at ${databasePath}` })).toBe("source_malformed"),
  );
  ...
  it("classifies an existing non-SQLite file as malformed", async () => {
    ...
    await expect(openReadOnlyDatabase(invalidPath, {
      protectedSourceRoots: [fixtureRoot],
    })).rejects.toMatchObject({ code: "source_malformed" });
  });

  it("maps busy errors raised during a read to source_busy", async () => {
    await expect(withReadOnlyDatabase(databasePath, () => {
      throw Object.assign(new Error("synthetic busy source"), { code: "SQLITE_BUSY" });
    }, { protectedSourceRoots: [fixtureRoot] })).rejects.toMatchObject({ code: "source_busy" });
  });

  it("does not include raw errors or private paths in diagnostics", () => {
    const diagnostic = toSafeDiagnostic(new Error(`failed at ${databasePath}`), "conversation-store", new Date("2026-08-28T00:00:00Z"));
