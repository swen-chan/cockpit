...
describe("browser-safe fixture contracts", () => {
  ...
  it("validates strict Task 5 profile and system-source DTOs", () => {
    expect(
      profileSummarySchema.safeParse({
        profile: "default",
        profileKind: "default",
        homeLabel: "~/.hermes",
    ...
    expect(systemSourceSchema.safeParse({ ...mockSystemSources[0], rawConfig: {} }).success).toBe(
      false,
    );
  ...
  it("keeps the Overview contract compact and rejects nested source content", () => {
    ...
    expect(
      overviewSnapshotSchema.safeParse({
        ...overview,
