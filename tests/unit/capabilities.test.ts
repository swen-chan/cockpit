...
describe("skills and tools adapters", () => {
  ...
  function fixture(): { context: HermesContext; home: string } {
    ...
    mkdirSync(path.join(home, "skills", "research", "github_pat_1234567890abcdef"), { recursive: true });
    writeFileSync(path.join(home, "config.yaml"), [
      "model:",
    ...
    writeFileSync(path.join(home, "skills", "research", "enabled-skill", "SKILL.md"), [
      "---",
      "name: enabled-skill",
    ...
    writeFileSync(path.join(home, "skills", "productivity", "disabled-skill", "SKILL.md"), [
      "---",
      "name: disabled-skill",
    ...
    writeFileSync(path.join(home, "skills", ".archive", "hidden-skill", "SKILL.md"), [
      "---",
      "name: hidden-skill",
      "description: Must stay hidden.",
      "---",
    ].join("\n"));
    writeFileSync(path.join(home, "skills", "research", "broken-skill", "SKILL.md"), "# no frontmatter");
    writeFileSync(path.join(home, "skills", "research", "github_pat_1234567890abcdef", "SKILL.md"), [
      "---",
  ...
  it("lists safe skill metadata, exclusions, status, and no bodies", async () => {
    ...
    writeFileSync(path.join(outside, "escaped", "SKILL.md"), "---\nname: escaped\ndescription: outside\n---\n");
    ...
    expect(source.collection?.items.find((item) => item.name === "enabled-skill")?.status).toBe("enabled");
    expect(source.collection?.items.find((item) => item.name === "disabled-skill")?.status).toBe("disabled");
    expect(source.collection?.items.find((item) => item.name === "broken-skill")?.status).toBe("error");
  ...
  it("loads one bounded skill body by opaque id and redacts sensitive text", async () => {
    ...
    await expect(readSkillPreview(context, "../../config.yaml")).rejects.toMatchObject({ code: "invalid_path" });
  });
  ...
  it("derives observable CLI toolset states from allowlisted config fields", async () => {
    ...
    expect(source.collection?.items.find((item) => item.name === "plugin-observer")?.category).toBe("Plugin");
    expect(source.collection?.items.find((item) => item.name === "file")?.description).toBe("Enabled by platform_toolsets.cli.");
    expect(source.collection?.items.find((item) => item.name === "vision")?.description).toBe("Disabled by agent.disabled_toolsets.");
    const serialized = JSON.stringify(source);
    expect(serialized).not.toContain(home);
