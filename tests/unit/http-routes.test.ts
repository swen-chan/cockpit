...
describe("GET-only route boundary", () => {
  it.each([
    [
      "overview",
      () => overviewRoute.GET(new Request("http://127.0.0.1:3000/api/overview?source=/private")),
    ],
    [
      "system",
      () => systemRoute.GET(new Request("http://127.0.0.1:3000/api/system?source=/private")),
    ],
    [
      "skill-manifest",
      () =>
        systemContextRoute.GET(
          new Request("http://127.0.0.1:3000/api/system/context?id=../SKILL.md"),
        ),
    ],
    [
      "conversation-store",
      () => conversationsRoute.GET(new Request("http://127.0.0.1:3000/api/conversations?limit=26")),
    ],
    [
      "conversation-store",
      () =>
        conversationDetailRoute.GET(new Request("http://127.0.0.1:3000/api/conversations/raw-id"), {
          params: Promise.resolve({ id: "raw-id" }),
        }),
    ],
    [
      "workspace",
      () => filesRoute.GET(new Request("http://127.0.0.1:3000/api/files?path=..%2Foutside")),
    ],
    [
      "workspace",
      () => filePreviewRoute.GET(new Request("http://127.0.0.1:3000/api/files/preview")),
    ],
