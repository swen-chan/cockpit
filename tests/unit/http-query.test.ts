...
describe("HTTP query boundary", () => {
  it("accepts only the documented conversation page parameters", () => {
    expect(parseConversationPageQuery(request("/api/conversations"))).toEqual({ cursor: null, limit: 5 });
    expect(parseConversationPageQuery(request("/api/conversations?cursor=cursor-Abc_123&limit=25")))
  ...
  it("validates opaque detail identifiers before service loading", () => {
    ...
    expect(() => parseConversationRequest(
      request("/api/conversations/raw-id"),
      "raw-id",
    )).toThrowError(expect.objectContaining({ code: "invalid_path" }));

    expect(parseSkillQuery(request("/api/system/context?id=skill-abcdef1234567890abcdef12")))
      .toBe("skill-abcdef1234567890abcdef12");
    expect(() => parseSkillQuery(request("/api/system/context?id=../SKILL.md")))
      .toThrowError(expect.objectContaining({ code: "invalid_path" }));
  });
  ...
  it("accepts bounded relative workspace paths and rejects traversal or absolute input", () => {
    ...
    expect(parsePreviewQuery(request("/api/files/preview?path=docs%2Fnotes.md"))).toBe("docs/notes.md");
    expect(() => parseDirectoryQuery(request("/api/files?path=docs&__proto__=ignored")))
      .toThrowError(expect.objectContaining({ code: "invalid_path" }));

    for (const path of ["..%252Foutside", "%2Fabsolute", ".env", "docs%2F..%2Foutside"]) {
      expect(() => parsePreviewQuery(request(`/api/files/preview?path=${path}`)))
        .toThrowError(expect.objectContaining({ code: expect.any(String) }));
    }
    expect(() => parsePreviewQuery(request("/api/files/preview")))
      .toThrowError(expect.objectContaining({ code: "invalid_path" }));
  });

  it("rejects query input on parameterless endpoints", () => {
    expect(() => assertNoQuery(request("/api/jobs"))).not.toThrow();
    expect(() => assertNoQuery(request("/api/jobs?source=/private/file")))
      .toThrowError(expect.objectContaining({ code: "invalid_path" }));
    for (const specialKey of ["__proto__", "constructor", "toString"]) {
      expect(() => assertNoQuery(request(`/api/jobs?${specialKey}=ignored`)))
        .toThrowError(expect.objectContaining({ code: "invalid_path" }));
    }
  });
});
